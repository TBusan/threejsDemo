...
function Contours() {
  ...
  var xCoords = null,  // 存储mockData中的x坐标数组
      yCoords = null;  // 存储mockData中的y坐标数组

  // 将mapCoordinate移到闭包内部
  function mapCoordinate(coord, axis) {
    const coords = axis === 'x' ? xCoords : yCoords;
    if (!coords) return coord;
    
    const n = coords.length;
    const i = Math.max(0, Math.min(coord, n - 1));
    const i0 = Math.floor(i);
    const i1 = i0 + 1;
    
    if (i1 >= n) return coords[n-1];
    if (i0 < 0) return coords[0];
    
    const t = i - i0;
    return coords[i0] * (1 - t) + coords[i1] * t;
  }
    ...
    isorings(values, v, function(ring) {
      ...
      if (xCoords && yCoords) {
        ring.forEach(point => {
          ...
          // 转换为真实坐标
          point[0] = mapCoordinate(gridX, 'x');
          point[1] = mapCoordinate(gridY, 'y');
        });
      }
...
contours.smooth = function(_) {
  ...
  contours.x = function(_) {
    return arguments.length ? (xCoords = slice.call(_), contours) : xCoords;
  };

  contours.y = function(_) {
    return arguments.length ? (yCoords = slice.call(_), contours) : yCoords;
  };
